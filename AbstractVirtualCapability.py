...
class CapabilityErrorException(Exception):
...
class AbstractVirtualCapability(Thread):
    ...
    def __handle_command(self, command: dict):
        ...
        if command["type"] == "trigger":
            ...
            try:
                ...
                error_string = "Subcapability {} threw an error: {}".format(command["capability"], e)
                error_string = error_string.replace("\"", "").replace("\\", "")
                ret["error"] = error_string
                exc_info = sys.exc_info()
                traceback.print_exception(*exc_info)
            ...
            try:
                ...
                self.sub_cap_server_callback[command["src"]] = {"error": command["error"]}
                # raise KeyError(f"Parameter not provided for Capability {command}")
            # a KeyError: 'Parameter not provided for Capability {\'type\': \'response\', \'capability\': \'TransferBlock\', \'device\': \'VirtualCopter@1\', \'src\': \'WallManager-TransferBlock-1710774517.013326\', \'error\': "Some Error occured in function TransferBlock: ValueError(\'Still got the Block 102 while waiting for block 103\')"}'
    ...
    def invoke_sync(self, capability: str, params: dict) -> dict:
        ...
        if "error" in ret:
            raise CapabilityErrorException(f"SubCap {capability} with params {params} failed: " + str(ret["error"]))
        return ret
    ...
    def invoke_async(self, capability: str, params: dict, callback) -> Thread:
        ...
        def __wait_for_callback(callback):
            ...
            if self.sub_caps_running:
                ret = self.sub_cap_server_callback.pop(src)
                callback(ret)
                if "error" in self.sub_cap_server_callback[src]:
                    raise CapabilityErrorException(f"SubCap {capability} with params {params} failed: " + str(ret["error"]))
...
class SubDeviceRepresentation(object):
    ...
    def invoke_sync(self, cap: str, params: dict):
        ...
        if "error" in ret:
            raise CapabilityErrorException(
                f"SubCap {cap} with params {params} failed: " + str(ret["error"]))
        return ret
    ...
    def invoke_async(self, cap: str, params: dict, callback):
        ...
        def __wait_for_callback(cb):
            ...
            ret = {}
            if self.master.sub_caps_running:
                ret = self.master.sub_cap_server_callback.pop(src)
                if "error" in ret:
                    raise CapabilityErrorException(
                        f"SubCap {cap} with params {params} failed: " + str(ret["error"]))
                cb(dict(ret))
